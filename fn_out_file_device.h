...
namespace FNLog
{
    ...
    inline void OpenFileDevice(Logger & logger, Channel & channel, Device & device, FileHandler & writer, LogData & log)
    {
        ...
        bool stuff_up = (bool)AtomicLoadC(device, DEVICE_CFG_FILE_STUFF_UP); //获取配置信息
    ...
    inline void EnterProcOutFileDevice(Logger& logger, int channel_id, int device_id, LogData& log)
    {
        ...
        AtomicAddL(device, DEVICE_LOG_TOTAL_WRITE_LINE);	// 行数加1
        AtomicAddLV(device, DEVICE_LOG_TOTAL_WRITE_BYTE, log.content_len_); //bytes 加长度
        AtomicAddLV(device, DEVICE_LOG_CUR_FILE_SIZE, log.content_len_); //文件大小加长度
    }
