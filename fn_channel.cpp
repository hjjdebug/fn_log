...
namespace FNLog
{
	...
	void EnterProcChannel(Logger& logger, int channel_id)
	{
		...
		do
		{
			...
			do
			{
				...
				AtomicAddL(channel, CHANNEL_LOG_PROCESSED); //将channel[CHANNEL_LOG_PROCESSED]加1
				local_write_count ++;
				do
				{
					//set read index to proc index  !! else, read_idex_++
	...
	bool BlockInput(Logger& logger, int channel_id, int priority, int category, long long identify) //阻止Input
	{
		...
		long long begin_category = AtomicLoadC(channel, CHANNEL_CFG_CATEGORY); //获取channel 一个配置值
	...
	int HoldChannel(Logger& logger, int channel_id, int priority, int category, long long identify)
	{
		...
		do
		{
			...
			state++;
			for (int i = 0; i < FN_MAX(RingBuffer::BUFFER_LEN, 10); i++) //RingBuffer 定义了1000个
			{
				if (channel.channel_state_ != CHANNEL_STATE_RUNNING)
				{
					break;
				...
				if (hold_idx == ring_buffer.read_idx_.load(std::memory_order_acquire)) //让hold_idx 等于read_idx_
				{
					break;
				}
...
#else
		static thread_local unsigned int thread_id = (unsigned int)syscall(SYS_gettid);
		log.thread_ = thread_id;
#endif
		log.content_[log.content_len_] = '\0';
